...
默认 fixture 包含 100 个客户端路由。每个路由由 1 个页面模块和 1,500 个叶子模块组成，只有入口和路由表两个模块共享：总计 150,102 个模块，其中 150,100 个（99.9987%）只属于单一路由。路由表使用显式动态 `import()`，webpack 和 Rspack 都开启按动态导入懒编译，Turbopack 使用其 dev server 的按请求编译路径。
